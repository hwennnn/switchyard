...
def check_public_docs() -> None:
    ...
    require("estimated tokens" in readme, "README should document agent context budget guardrail")
    ...
    testpypi_smoke = release_workflow.split("- name: TestPyPI install smoke", 1)[1].split("\n  publish-pypi:", 1)[0]
    require("python -m pip install --no-cache-dir" in testpypi_smoke, "TestPyPI smoke should install the published package")
    require("--index-url https://test.pypi.org/simple/" in testpypi_smoke, "TestPyPI smoke should use the TestPyPI index")
    require("--extra-index-url https://pypi.org/simple/" in testpypi_smoke, "TestPyPI smoke should allow PyPI dependency fallback")
    require("switchyard doctor --json" in testpypi_smoke, "TestPyPI smoke should validate installed doctor JSON")
    require("name = \"testpypi-smoke\"" in testpypi_smoke, "TestPyPI smoke should create a configured project")
    require("switchyard mcp config --json" in testpypi_smoke, "TestPyPI smoke should validate MCP config JSON")
    require('data["args"][-2:] == ["--project", "switchyard"]' in testpypi_smoke, "TestPyPI smoke should require alias-based MCP args")
    require("[mcp_servers.switchyard.env]" in testpypi_smoke, "TestPyPI smoke should validate MCP env table")
    require("cwd =" in testpypi_smoke and "not in data[\"config_text\"]" in testpypi_smoke, "TestPyPI smoke should reject cwd MCP config")
    require("switchyard mcp projects --json" in testpypi_smoke, "TestPyPI smoke should validate MCP alias registry")
    require("switchyard mcp smoke --json" in testpypi_smoke, "TestPyPI smoke should run the reusable MCP smoke")
    require("switchyard skill show" in testpypi_smoke, "TestPyPI smoke should validate bundled skill output")
...
def check_benchmark() -> None:
    ...
    require(metrics["brief_json"]["estimated_tokens"] < 3000, "brief output token estimate is too large for agent context")
